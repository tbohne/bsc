...
um eine Kombination zweier Zielfunktionen. Zu Beginn geht es darum, den zur Verfügung stehenden Platz in
der Storage-Area durch initiales Generieren möglichst vieler $b$-Tupel von Items, ökonomisch zu nutzen.
Dabei werden noch keine Kosten minimiert. Erst im Anschluss werden diese $b$-Tupel möglichst günstig
den zur Verfügung stehenden Stacks zugewiesen.

Ein ausschließlicher Fokus auf Kostenminimierung nachdem die Zulässigkeit erreicht ist, wäre umsetzbar,
indem zuletzt ausgewählte $b$-Tupel von Items wieder separiert und jeweils eigenen Stacks zugewiesen würden,
solange freie Stacks zur Verfügung stünden und sich infolgedessen Kosten einsparen ließen.
Davon wird in dieser Arbeit abgesehen, da es in der Praxis durchaus sinnvoll ist, den zur Verfügung stehenden Platz möglichst ökonomisch zu nutzen und nicht bedingungslos Kosten zu minimieren und dabei viele lediglich partiell gefüllte Stacks zu generieren.
...
Die erste vom \textsc{TestDataGenerator} bereitgestellte Konfigurationsmöglichkeit ist die Größe des Spielraums in der Storage-Area, welche die Anzahl der zur Verfügung stehenden Stacks beeinflusst.
Die Anzahl der Items $n$ sowie die Stack-Kapazität $b$ wird stets spezifiziert und in der Berechnung der Anzahl zur Verfügung stehender Stacks $m$ verwendet.
Es gilt $m$ = $\ceil{\floor{\frac{n}{b}}$ + $20 \%}$. Bei einer Stackanzahl von $m$ = $\floor{\frac{n}{b}}$ gäbe es überhaupt keinen Spielraum bei der Itemplatzierung, d.h. die Itemanzahl wäre identisch mit der Anzahl
der verfügbaren Stackpositionen. Aufgrund der in Abschnitt \ref{sec:constraints} geschilderten Nebenbedingungen wären die meisten Instanzen dann jedoch unlösbar. Daher wurde für die durchgeführten Experimente ein durchaus geringer Spielraum von $20 \%$ eingeführt, der in weiteren Betrachtungen erhöht werden könnte.
...
dass ein Item $i$ in einem Stack $q$ platziert werden darf, in der gegenwärtigen Konfiguration $70 \%$ beträgt,
da in der Praxis tendenziell mehr Zuweisungen erlaubt als verboten sind.

\textbf{Transportkosten}

Die Transportkosten eines Items, also die Kosten der Verladung eines Items in einen Stack, entsprechen,
wie in Abschnitt \ref{sec:transport_costs} erläutert, der auf der Manhattan-Metrik
basierenden Distanz zwischen diesem Item und dem jeweiligen Stack.
Im Wesentlichen liegen der Transportkostenbestimmung zwei Grids zugrunde. Zum einen die Storage-Area,
welche die Stacks beinhaltet und zum anderen die Fahrzeuge, auf denen die Items geliefert werden.
...
zwischen Storage-Area und Fahrzeug gegeben.

Daraus ergeben sich mehrere Konfigurationsmöglichkeiten, die unterschiedliche Auswirkungen haben.
Zunächst geht es um die Konfiguration der Zellengröße der beiden Grids. Der als erstes weltweit standardisierte
Seecontainer ist der 20-Fuß-Container TEU\footnote{Twenty Foot Equivalent Unit (Standardcontainer).},
der bis heute international als Maßeinheit
gebräuchlich ist. Der TEU ist rund $6,06$ Meter lang, $2,44$ Meter breit und $2,59$ Meter hoch (vgl. \citet{ContainerBasis}).
In grober Orientierung am TEU beträgt die Länge einer Zelle in den Experimenten $6$ Meter und deren Breite $3$ Meter.
Ebenfalls konfigurierbar ist die erwähnte Distanz zwischen den beiden Grids, welche in den Experimenten dem
$5-$fachen der Zellenbreite, also in diesem Fall $15$ Metern entspricht.
...
Die Länge eines Items wird dementsprechend zufällige aus dem Intervall $[1, 6]$ generiert und die Breite aus dem
Intervall $[1, 3]$. Wobei die konkreten Maße eines Items für die Transportkosten keine Rolle spielen und lediglich für die Stacking-Constraints von Bedeutung sind. Die in Variante 2 der Stacking-Constraint-Generierung erwähnten zufälligen
Längen und Breiten der Items stammen folglich aus diesen Intervallen.
Damit haben die Zellengrößen direkten Einfluss auf die Stacking-Constraints, weil kleinere Intervalle
z.B. dazu führen, dass die Wahrscheinlichkeit für Items, welche in beide Richtungen stackbar sind, größer ist.
