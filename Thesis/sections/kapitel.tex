...
Um den zusätzlichen Möglichkeiten und Anforderungen der erhöhten Stack-Kapazität gerecht zu werden, werden allerdings
auch erweiternde Ansätze entwickelt.
...
ebensolche Tripel von kompatiblen Items zu generieren. Dazu werden Item-Paare zu Item-Tripeln gemergt. Zunächst muss allerdings
sinnvoll entschieden werden, welche der zuvor generierten Item-Paare nun wieder separiert werden sollen, um beide Items
jeweils einem anderen Paar zuzuweisen und zwei Tripel zu bilden. Dies geschieht mithilfe eines Rating-Systems, welches die Item-Paare
mit dem Ziel bewertet, jene Paare zu ermitteln, die im Hinblick auf ein günstiges Ergebnis separiert werden sollten.
...
Wenn beispielsweise ein Item $i \in I$ einem Paar $\{s, t\} \in M$ zugewiesen werden soll, ergeben sich die folgenden $3!$
möglichen Permutationen eines Tripels.
\[
\begin{bmatrix}
...
\end{bmatrix}
\]
Sind die Items $s, t \in I$ in beide Richtungen stackbar, also $s_{st} + s_{ts} = 2$, so sind sämtliche Permutationen zu überprüfen.
Aufgrund der Transitivität der Stacking-Constraints ist das Item $i$ in diesem Fall kompatibel zum Paar $(s, t)$,
wenn $s_{is} + s_{it} + s_{si} + s_{ti} \geq 1$ gilt.
Gilt dagegen $s_{st} = 1$ und $s_{ts} = 0$, so müssen lediglich die ersten drei Permutationen überprüft werden.
Item $i$ ist dann kompatibel zum Paar $(s, t)$, wenn $s_{is} + s_{ti} \geq 1$ oder $s_{si} + s_{it} = 2$ gilt.
Der letzte Fall entspricht der Situation, in der $s_{st} = 0$ und $s_{ts} = 1$ gilt. Dort müssen lediglich die letzten drei
Permutationen überprüft werden und Item $i$ ist kompatibel zum Paar $(s, t)$, wenn $s_{it} + s_{si} \geq 1$ oder $s_{ti} + s_{is} = 2$ gilt.
...
Anschließend wird über sämtliche Paare hinweg eine durchschnittliche \textquote{Worst-Compatibility}, also eine durchschnittliche
Kompatibilität des jeweils weniger kompatiblen Items, sowie ein durchschnittlicher Kostenwert berechnet.
Wenn ein Paar nun sowohl eine über dem Durchschnitt liegende \textquote{Worst-Compatibility} als auch einen über dem Durchschnitt
liegenden Kostenwert besitzt, scheint es gut geeignet für eine Separation, denn es ist überdurchschnittlich flexibel
für eine erneute Zuweisung und seine gegenwärtige Zuweisung ist ohnehin überdurchschnittlich teuer.
...
überschreitet, entspricht das Rating ebendieser Summe. Dies sorgt dafür, dass auch die Höhe der Abweichungen einen
Einfluss auf die Bewertung der Paare hat.

Diese Schwelle, die im Folgenden als \textquote{Deviation-Threshold} bezeichnet wird, wird dabei in einem bestimmten Wertebereich
...
Dabei wird der \textquote{Deviation-Threshold} in jeder Iteration um $5$ inkrementiert. Es entstehen demnach 13 unterschiedlich bewertete Itemlisten zusätzlich zum ursprünglichen Ergebnis des \textsc{MCM}, welches unbeweret und unsortiert betrachtet wird. Diese Listen werden absteigend basierend auf ihrer Bewertung sortiert.
...
prozentualen Abweichung vom Durchschnitt in der Summe entspricht. Die dabei entstehenden einzelnen Items werden den verbleibenden $|M| - x$ Paaren zugewiesen um kompatible Tripel zu bilden. Die größte Anzahl an Item-Paaren, die separiert und dessen Items theoretisch den verbleibenden
Paaren zugewiesen werden könnten, beträgt $\floor{\frac{|M|}{3}}$. Aufgrund der Stacking Constraints ist es jedoch nicht sinnvoll, die Minimalanzahl verbleibender Paare zu wählen. Dementsprechend sollte $|M|$ durch einen Wert $> 3$ dividiert werden. Gleichzeitig sollte der Wert nicht zu groß gewählt werden, damit möglichst viele Tripel entstehen. Experimentell hat sich $3.4$ als guter Divisor herausgestellt, indem die Lösungen für unterschiedliche $x$ Werte verglichen wurden. Die ersten $x = \floor{\frac{|M|}{3.4}}$ Paare werden also separiert.
...
Dies ist nur dann möglich, wenn die Anzahl der Tripel plus die Anzahl der Paare plus die Anzahl der \textquote{Unmatched Items} kleiner
oder gleich der Anzahl der zur Verfügung stehenden Stacks ist.

Ist dies der Fall, so wird zu diesem Zweck ein vollständig bipartiter Graph generiert, welcher die Items, bestehend aus Tripeln, Paaren und Unmatched Items, in der einen Partition und die Stacks in der anderen Partition enthält.
Da die beiden Partitionen nicht notwendigerweise dieselbe Kardinalität besitzen, der verwendete Algorithmus zur Berechnung des \textsc{MWPM} dies jedoch fordert, werden Dummy-Items eingeführt, die die Item-Partition gegebenenfalls auffüllen. Diese Items haben keinen Einfluss auf die Lösung und werden in späteren Schritten ignoriert.
