...
zur Verfügung stehenden Stacks von $m = 4$ ergibt sich basierend auf der Beschreibung aus Abschnitt \ref{sec:test_data}.
...
\begin{tikzpicture}[scale=0.85, transform shape, node distance=2cm]
        ...
        \node[state] (C) [thick, right of=B] {$\boldsymbol{4, 6}$};
        \node[state] (D) [thick, right of=C] {$\boldsymbol{5, 7}$};
        \node[state] (E) [thick, right of=D] {$\boldsymbol{8, 9}$};
        \path;
\end{tikzpicture}
\caption{\textsc{Resultierende Item-Paare}.}
...
In diesem Fall ergeben sich aufgrund der geringen Anzahl an Paaren jedoch lediglich zwei voneinander unterschiedliche Sortierungen
(vgl. Abb. \ref{fig:lists_of_pairs}).
