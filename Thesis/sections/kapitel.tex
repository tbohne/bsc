...
Zwei Items $i, j$ sind in beide Richtungen stackbar, wenn $l_i = l_j$ und $w_i = w_j$ gilt.

Die zweite Variante erscheint realitätsnäher und wird deshalb in den Experimenten verwendet.
Aufgrund der Tatsache, dass diese jedoch stets zu einem Anteil von $1$-Einträgen in der
Stacking-Constraint-Matrix $S$ von ca. $25\%$ führt und jener mit Variante 1 sehr gut konfigurierbar ist,
wird Variante 1 nicht verworfen und kann ggf. in weiteren Experimenten, in welchen eine solche Konfiguration
erwünscht ist, zum Einstaz kommen.
...
Die partiellen Ordnungen lassen sich auch als Permutationen darstellen, wobei eine Kante zwischen
...
\ref{sec:transport_costs} erläutert, der auf der Manhattan-Metrik basierenden Distanz
zwischen der Originalposition dieses Items und der Position des jeweiligen Stacks.
...
In grober Orientierung am TEU betragen Länge und Breite einer Zelle in den Experimenten $6$ bzw. $3$ Meter.
Ebenfalls konfigurierbar ist die erwähnte Distanz zwischen den beiden Grids, welche in den Experimenten dem
Fünffachen der Zellenbreite, also in diesem Fall $15$ Metern entspricht.
...
Die Länge eines Items wird dementsprechend zufällige aus dem Intervall $[1, 6]$ generiert während die Breite aus dem
Intervall $[1, 3]$ stammt. Die konkreten Maße der Items sind für die Transportkosten irrelevant und lediglich
für die Stacking-Constraints von Bedeutung.
In der zweiten Variante der Stacking-Constraint-Generierung werden zufällige Längen und Breiten der Items
erwähnt, welche folglich aus diesen Intervallen stammen.
Somit haben die Zellengrößen direkten Einfluss auf die Stacking-Constraints,
weil die Wahrscheinlichkeit für Item-Paarungen, welche in beide Richtungen stackbar sind,
