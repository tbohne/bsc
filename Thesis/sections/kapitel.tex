...
In diesem Abschnitt werden MIP-Formulierungen eingeführt, die zum experimentellen Vergleich genutzt werden.
Diese werden verwendet, um die optimalen Zielfunktionswerte der Test-Instanzen zu ermitteln und somit eine Grundlage zu schaffen, mit der die Qualität der Lösungen der Heuristiken beurteilt werden kann. Des Weiteren geht es auch um einen Vergleich der im Folgenden vorgestellten MIP-Formulierungen untereinander. Zunächst wird in Abschnitt \ref{sec:mip_definition} eine kurze Einführung zu MIPs geliefert.
...
zu lösen. Es handelt sich dabei um eine u.a. auf dem Simplexverfahren basierende Implementation von IBM. \cite{CPLEX2015}
...
die diesen Behältern zugeordnet werden, wobei die Stack- also Behälterkapazität jeweils eingehalten wird. Im Unterschied zum klassischen \textsc{Bin-Packing} müssen zusätzlich die Stacking- und Placement-Constraints berücksichtigt werden.
...
(\ref{bin_packing_line_one}) definiert die Zielfunktion, die der Minimierung der Transportkosten entspricht. In (\ref{bin_packing_line_two}) wird
gefordert, dass jedes Item genau einem Stack zugewiesen wird. Die Ungleichung in (\ref{bin_packing_line_three}) entspricht der Forderung, dass die Stack-Kapazität eingehalten werden muss und (\ref{bin_packing_line_four}) entspricht schließlich der Forderung, dass alle Items, die nicht stackbar sind, nicht Teil desselben Stacks sind. Außerdem handelt es sich laut (\ref{bin_packing_line_five}) bei den $x_{iq}$ um Binärvariablen, bei denen $x_{iq} = 1$ bedeutet, dass Item $i$ Stack $q$ zugewiesen wird und $x_{iq} = 0$, wenn dies nicht der Fall ist.
