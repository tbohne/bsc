...
Insgesamt werden auf diese Weise so wenig Stacks wie möglich verwendet, denn auch wenn das eigentliche Ziel der Minimierung die Transportkosten
sind, steht zunächst stets im Vordergrund, überhaupt eine zulässige Lösung zu finden, weshalb die Maximierung der Anzahl der vollständig gefüllten Stacks initial eine gute Strategie darstellt.
...
Da die Placement-Constraints nicht direkt durch ein Fehlen der entsprechenden Kante in $G_1$, sondern
indirekt durch ein hohes Gewicht ebendieser Kante umgesetzt werden, existieren Kanten zwischen sämtlichen Knoten
aus $V_1$ und denen aus $V_2$, also $\forall i \in V_1, q \in V_2 \thinspace \exists \thinspace \{i, q\} \in A$.
Das Gewicht $c_{iq}$ dieser Kante ist definiert als die Kosten
des Transports des Items $i$ von seiner Originalposition $O_i$ auf dem Fahrzeug, mit dem es geliefert wird, zum Stack $q$
...
Da die Implementation des verwendeten Algorithmus zur Berechnung des \textsc{MWPM} in $G_1$ einen vollständig bipartiten Graphen
mit gleich großen Partitionen als Eingabe erwartet und die beiden Partitionen nicht notwendigerweise dieselbe Größe besitzen,
werden Dummy-Items eingeführt. Diese Items haben keinen Einfluss auf die Lösung, da sämtliche Kanten, die ein Dummy-Item mit einem Stack verbinden das Gewicht $0$ besitzen und somit kein echtes Item oder Item-Paar durch ein Dummy-Item von einer günstigeren Zuweisung abgehalten wird.
