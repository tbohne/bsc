...
Berechnet man nun ein \textsc{MCM} in $G$ und interpretiert dessen Kanten als Paare von kompatiblen Items, so erhält man
eine größtmögliche Anzahl von Item-Paaren, die in mindestens einer Richtung stackbar sind. In der Regel ist es möglich,
dass dabei sämtliche Items einer Instanz Teil eines Paares sind, denn typischerweise ist die Anzahl der Items gerade.
Dies hängt allerdings von den gegebenen Stacking-Constraints ab. In der zweiten Variante der Stacking-Constraint-Generierung,
die in Kapitel \ref{sec:test_data} beschrieben wird und die in sämtlichen Experimenten zum Einsatz kommt,
verbleiben in der Regel an dieser Stelle keine Unmatched-Items.
Sei $p = |MCM|$ nun als die Anzahl der resultierenden Item-Paare definiert.

Da die Stacks nun jedoch nicht nur Paare, sondern Tripel von Items beinhalten können, geht es im Folgenden darum,
eben solche Tripel von kompatiblen Items zu generieren. Dazu werden Item-Paare zu Item-Tripeln gemergt. Zunächst muss allerdings
...
mit dem Ziel bewertet, jene Paare zu ermitteln, die spariert werden sollten um zu einem möglichst günstigen Ergebnis zu gelangen.
...
Eine Menge von Items ist miteinander kompatibel, wenn diese basierend auf den Stacking-Constraints in mindestens einer
...
Anschließend wird über alle Paare hinweg die durchnittliche \textquote{Worst-Compatibility}, also die durchschnittliche
Kompatibilität des weniger kompatiblen Items, sowie der durchschnittliche Kostenwert berechnet.
Wenn ein Paar nun eine über dem Durchschnitt liegende \textquote{Worst-Compatibility} sowie einen über dem Durchschnitt
