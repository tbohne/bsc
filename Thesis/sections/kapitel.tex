...
enthält eine Kante $\{i, j\}$ zwischen zwei Knoten, wenn die entsprechenden Items $i, j \in I$ basierend auf den Stacking-Constraints
...
Um den zusätzlichen Möglichkeiten und Anforderungen der erhöhten Stack-Kapazität gerecht zu werden, werden hingegen
auch erweiternde Ansätze präsentiert.
...
Dies hängt allerdings auch von den gegebenen Stacking-Constraints ab und gilt nur für die zweite Variante der Stacking-Constraint-Generierung,
die in Kapitel \ref{sec:test_data} beschrieben wird. Da diese jedoch realitätsnäher zu sein scheint und daher auch in sämtlichen Experimenten zum Einsatz kommt, ist davon auszugehen, dass an dieser Stelle keine Unmatched-Items verbleiben.
...
ebensolche Tripel von kompatiblen Items zu generieren. Dazu werden die generierten Item-Paare zu Item-Tripeln gemergt.
Zunächst muss allerdings entschieden werden, welche der zuvor generierten Item-Paare nun sinnvollerweise wieder separiert werden sollten,
um beide Items jeweils einem anderen Paar zuzuweisen und zwei Tripel zu bilden. Dies geschieht mithilfe eines Rating-Systems, welches die Item-Paare mit dem Ziel bewertet, jene Paare zu ermitteln, die im Hinblick auf ein günstiges Ergebnis separiert werden sollten.
...
Wird beispielsweise überprüft, ob ein Item $i \in I$ einem Paar $(s, t)$, welches aus einer Kante $\{s, t\} \in M$ resultiert,
...
\end{figure}
Sind die Items $s, t \in I$, welche das Paar bilden, in beide Richtungen stackbar, also $s_{st} + s_{ts} = 2$, so sind sämtliche
...
Auf diese Weise wird die Kompatibilität sämtlicher Items zu den verbleibenden Paaren bestimmt.
Ausschlaggebend ist dabei die Anzahl der kompatiblen Paare des weniger kompatiblen Items eines Paares,
...
Wenn die Summe der prozentualen Abweichungen beider Werte $d_{ij}$ des Paares $(i, j)$ vom Durchschnitt eine gewisse Schwelle,
...
Die Summe der Abweichungen $d_{ij}$ ist wie folgt definiert:
\[ d_{ij} = \frac{|avgC - avgC_{ij}|}{avgC * 100} + \frac{|w - w_{ij}|}{w * 100}\]
Das Rating des Paares $(i, j)$ ergibt sich dementsprechend wie folgt:
\[
    r_{ij} \quad := \quad
\begin{cases}
...
\end{cases}
\]
Die Tatsache, dass das Rating $r_{ij}$ eines Paares $(i, j)$ direkt der Summe der Abweichungen $d_{ij}$ entspricht,
sofern diese die Schwelle $d_t$ überschreitet, hat zur Folge, dass auch die Höhe der Abweichungen einen Einfluss auf
die Bewertung der Paare hat.
...
in dem für diese Items generierten Stacking-Constraint-Graphen geschieht.
Items, welche nicht inzident zu einer Kante dieses Matchings sind, werden im Folgenden als unmatched betrachtet und auf dem Ground-Level
eines eigenen Stacks platziert.

Wie bereits in der Heuristik aus Abschnitt \ref{sec:two_cap_heuristic} werden auch hier insgesamt auf diese Weise möglichst wenig der zur Verfügung stehenden Stacks verwendet,
denn auch hier ist das primäre Ziel, zu einer zulässigen Lösung zu gelangen, wofür sich die initiale Maximierung der Anzahl der möglichst
weit gefüllten Stacks anbietet. Erst wenn möglichst viele Tripel und Paare generiert wurden, geht es darum, diese zusammen mit den verbleibenden Unmatched-Items möglichst günstig den vorhandenen Stacks zuzuweisen.
...
Dies ist nur dann möglich, wenn die Anzahl der Item-Tripel, die Anzahl der Item-Paare und die Anzahl der \textquote{Unmatched Items}
in der Summe kleiner oder gleich der Anzahl der zur Verfügung stehenden Stacks ist.

Ist dies der Fall, so wird zu diesem Zweck ein vollständig bipartiter Graph $G_1 = (V_1 \cup V_2, A)$ eingeführt,
dessen erste Partition $V_1$ die Items bestehend aus Item-Tripeln, Item-Paaren und Unmatched-Items enthält während die zweite Partition $V_2$
die zur Verfügung stehenden Stacks beinhaltet.
Eine Kante $\{i, q\} \in A$ zwischen einem Knoten $i \in V_1$ und einem Knoten $q \in V_2$ gibt an, dass das entsprechende Item,
Item-Parr oder Item-Tripel $i$ dem jeweiligen Stack $q$ zugeordnet werden kann.
Wie gehabt gilt dabei, dass die Placement-Constraints indirekt durch ein hohes Gewicht einer solchen Kante und nicht durch ein Fehlen dieser
umgesetzt werden.
Es existieren daher Kanten zwischen sämtlichen Knoten aud $V_1$ und denen aus $V_2$, also $\forall i \in V_1, q \in V_2
\exists \{i, q\} \in A$. Das Gewicht einer solchen Kante ist definiert als die Kosten $c_{iq}$ des Transports des Items $i$ von seiner
Originalposition $O_i$ auf dem Fahrzeug, mit dem es geliefert wird, zum Stack $q$ in der Storage-Area.
...
Im Fall eines Item-Paars $(i, j) \in V_1$ entspricht das Gewicht der Kante $\{(i, j), q\} \in A$ der Summe der Kosten des Transports
beider Items, also $c_{(i, j)q} = c_{iq} + c_{jq}$.

Im Fall eines Item-Tripels $(i, j, k) \in V_1$ entspricht das Gewicht der Kante $\{(i, j, k), q\} \in A$ der Summe der Kosten
des Transports der drei Items, also $c_{(i, j, k)q} = c_{iq} + c_{jq} + c_{kq}$.

Da die Partitionen $V_1$ und $V_2$ nicht notwendigerweise dieselbe Kardinalität besitzen, der von \textit{JGrpahT} bereitgestellte
Algorithmus, welcher zur Berechnung des \textsc{MWPM} in $G_1$ verwendet wird, dies jedoch fordert, werden Dummy-Items
eingeführt, die die Item-Partition $V_1$ gegebenenfalls auffüllen.
...
Nachdem beide Partitionen dieselbe Größe besitzen, wird ein \textsc{MWPM} in $G_1$ berechnet, dessen Kanten
als Stack-Zuweisungen interpretiert werden. Da es sich um ein Perfect-Matching handelt, werden sämtliche Items und Item-Paare
einem Stack zugewiesen. Die Minimum-Weight-Variante eines Perfect-Matchings sorgt dafür, dass es sich um eine günstigste Zuweisung
sämtlicher Items, Item-Paare und Item-Tripel zu Stacks handelt, da die Kantenkosten jeweils den Transportkosten der entsprechenden
Items entsprechen. Sollte das \textsc{MWPM} eine Kante der Kosten $\infty$ enthalten, so existiert keine zulässige Zuweisung
der generierten Tripel, Paare und Unmatched-Items zu den Stacks, welche die Placement-Constraints respektiert.

Auf diese Weise entstehen nun 14 Lösungen, von denen schließlich die günstigste ausgewählt wird.
